...
{
    public class DCLVideoRealtimeTexture : DCLTexture
    {
        public static bool VERBOSE = false;
        public static Logger logger = new Logger("DCLVideoRealtimeTexture") { verboseEnabled = VERBOSE };
        ...
        protected Coroutine texturePlayerUpdateRoutine;
        ...
        protected string lastVideoClipID;
        private VideoState previousVideoState;

        public DCLVideoRealtimeTexture()
        {
            model = new Model();
        ...
        public override int GetClassId() { return (int)CLASS_ID.VIDEO_REALTIME_TEXTURE; }
        ...
        public override IEnumerator ApplyChanges(BaseModel newModel)
        {
            ...
            {
                DCLVideoRealtimeStream dclVideoClip = scene.componentsManagerLegacy.GetSceneSharedComponent(lastVideoClipID) as DCLVideoRealtimeStream;
        ...
        private void Initialize(DCLVideoRealtimeStream dclVideoClip)
        {
            string videoId = (!string.IsNullOrEmpty(scene.sceneData.id)) ? scene.sceneData.id + id : scene.GetHashCode().ToString() + id;
            texturePlayer = new WebVideoPlayer(videoId, dclVideoClip.GetUrl(), false, true, DCLVideoTexture.videoPluginWrapperBuilder.Invoke());
            ...
            SetupRealtimeVideo(dclVideoClip.GetUrl(), dclVideoClip.GetStreamId(), dclVideoClip.GetProvider(), videoId);
        }

        private void SetupRealtimeVideo(string url, string streamId, string provider, string videoId)
        {
            WebInterface.SetupRealtimeVideo(url, streamId, provider, videoId);
        }

        private void UnloadRealtimeVideo(string videoId)
        {
            WebInterface.UnloadRealtimeVideo(videoId);
        }

        public float GetVolume() { return ((Model)model).volume; }

        protected IEnumerator OnUpdate()
        {
            while (true)
            {
                UpdateDirtyState();
        ...
        protected void ReportVideoProgress()
        {
        ...
        protected void OnPlayerCoordsChanged(Vector2Int coords, Vector2Int prevCoords)
        {
            SetPlayStateDirty();
        }

        protected void OnSceneIDChanged(string current, string previous) { isPlayerInScene = IsPlayerInSameSceneAsComponent(current); }
        ...
        protected void SetPlayStateDirty(IDCLEntity entity = null)
        {
            isPlayStateDirty = true;
        }

        protected void OnAudioSettingsChanged(AudioSettings settings) { UpdateVolume(); }

        public override void Dispose()
        {
            this.UnloadRealtimeVideo(this.lastVideoClipID);
